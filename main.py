...
def main_menu(message):
    ...
    markup.add("Подробнее рассказать о прачечной 📜")
    photo_url = 'https://ltdfoto.ru/image/meix0s'
        # Отправляем изображение
    bot.send_photo(message.chat.id, photo_url, caption="Здравствуйте 👋!\n"
                                                       "\n"
                                                       "Я - Чистлана 🌸, бот прачечной 'Просто стирка' и Ваш персональный ассистент в мире чистоты! 💁‍♀️\n"
                                      "\n"
...
def consulting_services(message):
    ...
    bot.send_message(message.chat.id, "Прачечная 'Просто стирка' занимаемся немалым спектром услуг в области стирки 🧺. В списке ниже Вы можете воспользоваться любой из представленных 👇.\n" 
            " \n"
            "🧺 Стирка одежды – аккуратно и с заботой о Ваших вещах.\n"
                                      "\n"
            "🛏 Стирка текстиля – шторы, постельное бельё и прочий уют.\n"
                                      "\n"
            "👟 Чистка кроссовок – вернём Вашим кроссовкам свежий вид и чистоту.\n"
            "\n"
            "🧼 Выведение пятен – сложные загрязнения нам не помеха."
                                      "\n"
...
def servises_services(message):
    ...
    markup.add("🛏 Стирка текстиля")
    markup.add("👟 Чистка кроссовок")
